...
// frappe.ui.form.on("Rental RentalSales Order", {
// 	refresh(frm) {
...
erpnext.accounts.taxes.setup_tax_validations("Rental Group Order");
erpnext.sales_common.setup_selling_controller();

frappe.ui.form.on("Rental Group Order", {
	...
	setup: function(frm) {
		...
		frm.set_indicator_formatter('item_code1',
	...
	refresh: function(frm) {
		if(frm.doc.docstatus === 1) {
			...
			// 			frm: frm,
			// 			child_docname: "items",
			// 			child_doctype: "RentalSales Order Detail",
		...
		// Hide `Reserve Stock` field description in submitted or cancelled RentalSales Order.
		if (frm.doc.docstatus > 0) {
	...
	onload: function(frm) {
		...
		frm.set_query('warehouse', 'items', function(doc, cdt, cdn) {
			...
			if (row.item_code1) {
				...
				query.filters.push(["Bin", "item_code1", "=", row.item_code1]);
			}
			return query;
		});

		// On cancel and amending a Rentalsales order with advance payment, reset advance paid amount
		if (frm.is_new()) {
		...
		const dialog = new frappe.ui.Dialog({
			...
			fields: [
				...
				{fieldtype: "Column Break"},
				{
					...
					fieldtype: "Link",
					label: __("Add Item"),
					options: "RentalSales Order Item",
					get_query: () => {
						return {
					...
					fields: [
						{
							...
							fieldtype: "Link",
							label: __("RentalSales Order Item"),
							options: "RentalSales Order Item",
							reqd: 1,
							in_list_view: 1,
							get_query: () => {
								...
								if (event) {
									...
									frm.doc.items.forEach(item => {
										if (item.name === item_row.sales_order_item) {
											item_row.item_code1 = item.item_code1;
										}
									});
									dialog.fields_dict.items.grid.refresh();
								}
...
frappe.ui.form.on("Rental Sales Order Item", {
	// item_code1: function(frm,cdt,cdn) {
	// 	var row = locals[cdt][cdn];
...
erpnext.selling.SalesOrderController = class SalesOrderController extends erpnext.selling.SellingController {
	...
	refresh(doc, dt, dn) {
		...
		if (doc.docstatus==1) {
			...
			if (doc.status === 'On Hold') {
				...
				this.frm.add_custom_button(__('Resume'), function() {
					frappe.call({
						method: 'nhk.nhk.doctype.rental_group_order.rental_group_order.update_status',
						args: {
			...
			if(doc.status !== 'Closed') {
				if(doc.status !== 'On Hold') {
					...
					// 	// Don't show the `Reserve` button if the RentalSales Order has Picked Items.
					...
					if(flt(doc.per_billed, 2) < 100 ) {
						this.frm.add_custom_button(__('Sales Invoice'), () => me.make_sales_invoice(), __('Action'));
					}
				...
				if(flt(doc.per_billed, precision('per_billed', doc)) < 100 + frappe.boot.sysdefaults.over_billing_allowance) {
					this.frm.add_custom_button(__('Payment Request'), () => this.make_payment_request(), __('Action'));
		...
		// 						${__("Note: Please create RentalSales Orders from individual Quotations to select from among Alternative Items.")}
		// 					</span>
		// 			`);
		// 			}, 200);
	...
	create_pick_list() {
		frappe.model.open_mapped_doc({
			method: "nhk.nhk.doctype.rental_group_order.rental_group_order.create_pick_list",
			frm: this.frm
		})
	}
	...
	// 	var me = this;
	// 	me.frm.call({
	// 		method: "nhk.nhk.doctype.rental_group_order.rental_group_order.get_work_order_items",
	// 		args: {
	...
	// 							return { filters: { item: doc.item_code1 } };
	// 						}
	// 					}, {
	// 						fieldtype: 'Float',
	...
	make_material_request() {
		frappe.model.open_mapped_doc({
			method: "nhk.nhk.doctype.rental_group_order.rental_group_order.make_material_request",
			frm: this.frm
		})
	}
	...
	// 			fields: [
	// 				{fieldtype:'Read Only', fieldname:'item_code1',
	...
	// 			me.frm.call({
	// 				method: 'nhk.nhk.doctype.rental_group_order.rental_group_order.make_raw_material_request',
	// 				args: {
	// 					items: data,
	// 					company: me.frm.doc.company,
	// 					sales_order: me.frm.docname,
	...
	// 	frappe.model.open_mapped_doc({
	// 		method: "nhk.nhk.doctype.rental_group_order.rental_group_order.make_delivery_note",
	// 		frm: this.frm,
	// 		args: {
	// 			delivery_dates,
	...
	make_sales_invoice() {
		frappe.model.open_mapped_doc({
			method: "nhk.nhk.doctype.rental_group_order.rental_group_order.make_sales_invoice",
			frm: this.frm
		})
	}
	...
	make_approved() {
        frappe.call({
            method: 'nhk.nhk.doctype.rental_group_order.rental_group_order.make_approved',
            args: {
	...
	close_rental_sales_order() {
        frappe.call({
            method: 'nhk.nhk.doctype.rental_group_order.rental_group_order.close_rental_order',
            args: {
                docname: this.frm.doc.name,
            },
            callback: function (response) {
				...
				frappe.call({
					...
					callback: function (r) {
						if (!r.exc) {
							...
							frappe.call({
								method: 'nhk.nhk.doctype.rental_group_order.rental_group_order.on_hold',
								args: {
									docname: me.frm.doc.name,
								},
								callback: function (response) {
    ...
    callServerMethod(values) {
        frappe.call({
            method: 'nhk.nhk.doctype.rental_group_order.rental_group_order.make_rental_device_assign',
            args: {
                docname: this.frm.doc.name,
    ...
    make_ready_for_delivery() {
        frappe.call({
            method: 'nhk.nhk.doctype.rental_group_order.rental_group_order.make_ready_for_delivery',
            args: {
                docname: this.frm.doc.name,
            },
            callback: (response) => {
...
callServerMethodForDispatch(values) {
    frappe.call({
        method: 'nhk.nhk.doctype.rental_group_order.rental_group_order.make_dispatch',
        args: {
            docname: this.frm.doc.name,
...
callServerMethodForDelivered(values) {
    frappe.call({
        method: 'nhk.nhk.doctype.rental_group_order.rental_group_order.make_delivered',
        args: {
...
callServerMethodForReadyForPickup(values) {
    frappe.call({
        method: 'nhk.nhk.doctype.rental_group_order.rental_group_order.make_ready_for_pickup',
        args: {
            docname: this.frm.doc.name,
...
callServerMethodForPickedup(values) {
    frappe.call({
        method: 'nhk.nhk.doctype.rental_group_order.rental_group_order.make_pickedup',
        args: {
            docname: this.frm.doc.name,
        ...
        callback: (response) => {
            ...
            if (response.message) {
                ...
                frappe.msgprint({
                    title: __('Success'),
                    message: __('RentalSales Order is picked up with technician details.'),
                    indicator: 'green'
                });
                // this.frm.save();
                // Reload the entire page after a short delay (adjust as needed)
...
callServerMethodForSubmittedToOffice(values) {
    const itemCodes = this.frm.doc.items.map(item => item.item_code1);
	console.log(itemCodes)
    frappe.call({
        method: 'nhk.nhk.doctype.rental_group_order.rental_group_order.make_submitted_to_office',
        args: {
            docname: this.frm.doc.name,
	...
	// 	frappe.model.open_mapped_doc({
	// 		method: "nhk.nhk.doctype.rental_group_order.rental_group_order.make_maintenance_schedule",
	// 		frm: this.frm
	// 	})
	// }
	...
	// make_project() {
	// 	frappe.model.open_mapped_doc({
	// 		method: "nhk.nhk.doctype.rental_group_order.rental_group_order.make_project",
	// 		frm: this.frm
	// 	})
	// }
	...
	// 	frappe.model.open_mapped_doc({
	// 		method: "nhk.nhk.doctype.rental_group_order.rental_group_order.make_inter_company_purchase_order",
	// 		frm: this.frm
	// 	});
	// }
	...
	// 	frappe.model.open_mapped_doc({
	// 		method: "nhk.nhk.doctype.rental_group_order.rental_group_order.make_maintenance_visit",
	// 		frm: this.frm
	// 	})
	// }
	...
	// 	})
	// 	if(!pending_items){
	// 		frappe.throw({message: __("Purchase Order already created for all RentalSales Order items"), title: __("Note")});
	// 	}
	...
	// 			return frappe.call({
	// 				method: "nhk.nhk.doctype.rental_group_order.rental_group_order." + method,
	...
	// 				if (pending_qty > 0) {
	// 					po_items.push({
	// 						"doctype": "Rental Sales Order Item",
	// 						"name": d.name,
	// 						"item_name": d.item_name,
	// 						"item_code1": d.item_code1,
