...
# class TestRentalGroupOrder(FrappeTestCase):
# 	pass
...
class TestRentalGroupOrder(FrappeTestCase):
	...
	def test_sales_order_with_negative_rate(self):
		"""
		Test if negative rate is allowed in Rental Group Order via doc submission and update items
		"""
		...
		update_child_qty_rate("Rental Group Order", trans_item, so.name)
	...
	def test_update_child_removing_item(self):
		...
		self.assertRaises(
			frappe.ValidationError, update_child_qty_rate, "Rental Group Order", trans_item, so.name
		)
	...
	def test_update_child_with_precision(self):
		...
		precision = get_field_precision(frappe.get_meta("Rental Sales Order Item").get_field("rate"))

		make_property_setter("Rental Sales Order Item", "rate", "precision", 7, "Currency")
		...
		make_property_setter("Rental Sales Order Item", "rate", "precision", precision, "Currency")
	...
	def test_material_request_for_product_bundle(self):
		# Create the Material Request from the Rental Group Order for the Packing Items
	...
	def test_update_child_product_bundle(self):
		...
		update_child_qty_rate("Rental Group Order", added_item, so.name)
		...
		update_child_qty_rate("Rental Group Order", update_uom_conv_factor, so.name)
	...
	def test_update_child_with_tax_template(self):
		...
		update_child_qty_rate("Rental Group Order", items, so.name)
	...
	def test_drop_shipping(self):
		...
		self.assertEqual(flt(frappe.db.get_value("Rental Group Order", so.name, "per_delivered"), 2), 100.00)
		po.load_from_db()
	...
	def test_product_bundles_in_so_are_replaced_with_bundle_items_in_po(self):
		"""
		Tests if the the Product Bundles in the Items table of Rental Group Orders are replaced with
	...
	def test_advance_payment_entry_unlink_against_sales_order(self):
		...
		pe = get_payment_entry("Rental Group Order", so.name, bank_account="_Test Bank - _TC")
		pe.reference_no = "1"
		...
		so_doc = frappe.get_doc("Rental Group Order", so.name)
	...
	def test_cancel_sales_order_after_cancel_payment_entry(self):
		...
		# create a payment entry against Rental Group Order
		pe = get_payment_entry("Rental Group Order", so.name, bank_account="_Test Bank - _TC")
		pe.reference_no = "1"
		pe.reference_date = nowdate()
		pe.paid_from_account_currency = so.currency
		...
		# Cancel Rental Group Order
		try:
			so_doc = frappe.get_doc("Rental Group Order", so.name)
			so_doc.cancel()
		except Exception:
			self.fail("Can not cancel Rental Group Order with linked cancelled payment entry")

	def test_work_order_pop_up_from_sales_order(self):
		"Test `get_work_order_items` in Rental Group Order picks the right BOM for items to manufacture."
	...
	def test_so_optional_blanket_order(self):
		"""
		Expected result: Blanket order Ordered Quantity should only be affected on Rental Group Order with against_blanket_order = 1.
		Second Rental Group Order should not add on to Blanket Orders Ordered Quantity.
		"""
		...
		so_doc = frappe.get_doc("Rental Group Order", so.get("name"))
	...
	def test_so_cancellation_when_si_drafted(self):
		"""
		Test to check if Rental Group Order gets cancelled if Sales Invoice is in Draft state
		Expected result: Rental Group Order should not get cancelled
		"""
		so = make_sales_order()
		so.submit()
		si = make_sales_invoice(so.name)
	...
	def test_so_cancellation_after_si_submission(self):
		"""
		Test to check if Rental Group Order gets cancelled when linked Sales Invoice has been Submitted
		Expected result: Rental Group Order should not get cancelled
		"""
		so = make_sales_order()
		so.submit()
	...
	def test_so_cancellation_after_dn_submission(self):
		"""
		Test to check if Rental Group Order gets cancelled when linked Delivery Note has been Submitted
		Expected result: Rental Group Order should not get cancelled
		"""
		so = make_sales_order()
		so.submit()
	...
	def test_so_cancellation_after_maintenance_visit_submission(self):
		...
		mv.purposes[0].prevdoc_doctype = "Rental Group Order"
	...
	def test_sales_order_partial_advance_payment(self):
		...
		# Create a advance payment against that Rental Group Order
		pe = get_payment_entry("Rental Group Order", so.name, bank_account="_Test Bank - _TC")
		pe.reference_no = "1"
		pe.reference_date = nowdate()
		pe.paid_from_account_currency = so.currency
	...
	def test_expired_rate_for_packed_item(self):
		...
		for scenario in [
			...
			with self.subTest(scenario=scenario):
				...
				so = frappe.new_doc("Rental Group Order")
				so.transaction_date = nowdate()
	...
	workflow = frappe.get_doc(
		{
			...
			"document_type": "Rental Group Order",
