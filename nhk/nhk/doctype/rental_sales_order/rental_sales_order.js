...
erpnext.accounts.taxes.setup_tax_validations("RentalSales Order");
erpnext.sales_common.setup_selling_controller();

frappe.ui.form.on("RentalSales Order", {
...
frappe.ui.form.on("RentalSales Order Item", {
...
erpnext.selling.SalesOrderController = class SalesOrderController extends erpnext.selling.SellingController {
		...
		if (doc.docstatus==1) {
			...
			if(doc.status !== 'Closed') {
				if(doc.status !== 'On Hold') {
                    ...
                    if (flt(doc.per_billed, 2) < 100 && doc.status === 'Rental Device Assigned' && doc.order_type === 'Rental') {
	...
	create_pick_list() {
		frappe.model.open_mapped_doc({
			method: "nhk.nhk.doctype.rental_sales_order.rental_sales_order.create_pick_list",
			frm: this.frm
		})
	}
	...
	make_work_order() {
		var me = this;
		me.frm.call({
			method: "nhk.nhk.doctype.rental_sales_order.rental_sales_order.get_work_order_items",
			args: {
	...
	make_material_request() {
		frappe.model.open_mapped_doc({
			method: "nhk.nhk.doctype.rental_sales_order.rental_sales_order.make_material_request",
			frm: this.frm
		})
	}
	...
	make_raw_material_request_dialog(r) {
		...
		var d = new frappe.ui.Dialog({
			...
			primary_action: function() {
				...
				me.frm.call({
					method: 'nhk.nhk.doctype.rental_sales_order.rental_sales_order.make_raw_material_request',
					args: {
						items: data,
						company: me.frm.doc.company,
						sales_order: me.frm.docname,
	...
	make_delivery_note(delivery_dates, for_reserved_stock=false) {
		frappe.model.open_mapped_doc({
			method: "nhk.nhk.doctype.rental_sales_order.rental_sales_order.make_delivery_note",
			frm: this.frm,
			args: {
				delivery_dates,
	...
	make_sales_invoice() {
		frappe.model.open_mapped_doc({
			method: "nhk.nhk.doctype.rental_sales_order.rental_sales_order.make_sales_invoice",
			frm: this.frm
		})
	}
    ...
    make_approved() {
        frappe.call({
            method: 'nhk.nhk.doctype.rental_sales_order.rental_sales_order.make_approved',
            args: {
    ...
    callServerMethod(values) {
        frappe.call({
            method: 'nhk.nhk.doctype.rental_sales_order.rental_sales_order.make_rental_device_assign',
            args: {
                docname: this.frm.doc.name,
    ...
    make_ready_for_delivery() {
        frappe.call({
            method: 'nhk.nhk.doctype.rental_sales_order.rental_sales_order.make_ready_for_delivery',
            args: {
                docname: this.frm.doc.name,
            },
            callback: (response) => {
...
callServerMethodForDispatch(values) {
    frappe.call({
        method: 'nhk.nhk.doctype.rental_sales_order.rental_sales_order.make_dispatch',
        args: {
            docname: this.frm.doc.name,
...
callServerMethodForReadyForPickup(values) {
    frappe.call({
        method: 'nhk.nhk.doctype.rental_sales_order.rental_sales_order.make_ready_for_pickup',
        args: {
            docname: this.frm.doc.name,
...
callServerMethodForPickedup(values) {
    frappe.call({
        method: 'nhk.nhk.doctype.rental_sales_order.rental_sales_order.make_pickedup',
        args: {
            docname: this.frm.doc.name,
...
callServerMethodForSubmittedToOffice(values) {
	...
	make_maintenance_schedule() {
		frappe.model.open_mapped_doc({
			method: "nhk.nhk.doctype.rental_sales_order.rental_sales_order.make_maintenance_schedule",
			frm: this.frm
		})
	}
	...
	make_project() {
		frappe.model.open_mapped_doc({
			method: "nhk.nhk.doctype.rental_sales_order.rental_sales_order.make_project",
			frm: this.frm
		})
	}
	...
	make_inter_company_order() {
		frappe.model.open_mapped_doc({
			method: "nhk.nhk.doctype.rental_sales_order.rental_sales_order.make_inter_company_purchase_order",
			frm: this.frm
		});
	}
	...
	make_maintenance_visit() {
		frappe.model.open_mapped_doc({
			method: "nhk.nhk.doctype.rental_sales_order.rental_sales_order.make_maintenance_visit",
			frm: this.frm
		})
	}
	...
	make_purchase_order(){
		...
		var dialog = new frappe.ui.Dialog({
			...
			primary_action (args) {
				...
				return frappe.call({
					method: "nhk.nhk.doctype.rental_sales_order.rental_sales_order." + method,
		...
		function set_po_items_data (dialog) {
			...
			if (against_default_supplier) {
				...
				me.frm.doc.items.forEach(d => {
					...
					if (pending_qty > 0) {
						po_items.push({
							"doctype": "RentalSales Order Item",
							"name": d.name,
	...
	// 	frappe.call({
	// 		method: "nhk.nhk.doctype.rental_sales_order.rental_sales_order.update_status",
