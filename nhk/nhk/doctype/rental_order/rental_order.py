...
class RentalOrder(SellingController):
	...
	def __init__(self, *args, **kwargs):
		super(RentalOrder, self).__init__(*args, **kwargs)
	...
	def validate(self):
		...
		# self.status = "Active"
		# super(RentalSalesOrder, self).validate()
	...
	def validate_po(self):
		...
		if self.po_no and self.customer and not self.skip_delivery_note:
			so = frappe.db.sql(
				"select name from `tabRental Sales Order` \
			...
			if so and so[0][0]:
				if cint(
					...
					frappe.throw(
						_(
							"Rental Sales Order {0} already exists against Customer's Purchase Order {1}. To allow multiple Rental Sales Orders, Enable {2} in {3}"
						).format(
							frappe.bold(so[0][0]),
	...
	def validate_warehouse(self):
		super(RentalOrder, self).validate_warehouse()

		for d in self.get("items"):
			if (
				(
					frappe.get_cached_value("Item", d.item_code1, "is_stock_item") == 1
					or (self.has_product_bundle(d.item_code1) and self.product_bundle_has_stock_item(d.item_code))
				)
				and not d.warehouse
	...
	def validate_with_previous_doc(self):
		super(RentalOrder, self).validate_with_previous_doc(
			{
	...
	def on_cancel(self):
		...
		super(RentalOrder, self).on_cancel()
	...
	def check_nextdoc_docstatus(self):
		linked_invoices = frappe.db.sql_list(
			...
			where t1.name = t2.parent and t2.rental_sales_order = %s and t1.docstatus = 0""",
			self.name,
		)
		...
		if linked_invoices:
			...
			frappe.throw(
				_("Sales Invoice {0} must be deleted before cancelling this Rental Sales Order").format(
					", ".join(linked_invoices)
				)
			)

	def check_modified_date(self):
		mod_db = frappe.db.get_value("Rental Sales Order", self.name, "modified")
		...
		for item in self.items:
			if item.supplier:
				supplier = frappe.db.get_value(
					"Rental Sales Order Item", {"parent": self.name, "item_code": item.item_code}, "supplier"
				)
	...
	def has_unreserved_stock(self) -> bool:
		...
		reserved_qty_details = get_sre_reserved_qty_details_for_voucher("Rental Sales Order", self.name)
...
def close_or_unclose_rental_sales_orders(names, status):
	if not frappe.has_permission("Rental Sales Order", "write"):
		frappe.throw(_("Not permitted"), frappe.PermissionError)

	names = json.loads(names)
	for name in names:
		so = frappe.get_doc("Rental Sales Order", name)
		if so.docstatus == 1:
			if status == "Closed":
...
def make_material_request(source_name, target_doc=None):
	...
	doc = get_mapped_doc(
		...
		source_name,
		{
			"Rental Sales Order": {"doctype": "Material Request", "validation": {"docstatus": ["=", 1]}},
			"Packed Item": {
			...
			"Rental Sales Order Item": {
				"doctype": "Material Request Item",
				"field_map": {"name": "rental_rental_sales_order_item", "parent": "rental_sales_order"},
...
def make_delivery_note(source_name, target_doc=None, kwargs=None):
	...
	if kwargs.for_reserved_stock:
		sre_details = get_sre_reserved_qty_details_for_voucher("Rental Sales Order", source_name)

	mapper = {
		"Rental Sales Order": {"doctype": "Delivery Note", "validation": {"docstatus": ["=", 1]}},
	...
	so = frappe.get_doc("Rental Sales Order", source_name)
	target_doc = get_mapped_doc("Rental Sales Order", so.name, mapper, target_doc)

	if not kwargs.skip_item_mapping and kwargs.for_reserved_stock:
		sre_list = get_sre_details_for_voucher("Rental Sales Order", source_name)
...
def make_maintenance_schedule(source_name, target_doc=None):
	...
	if not maint_schedule:
		doclist = get_mapped_doc(
			...
			source_name,
			{
				"Rental Sales Order": {"doctype": "Maintenance Schedule", "validation": {"docstatus": ["=", 1]}},
				"Rental Sales Order Item": {
...
def make_maintenance_visit(source_name, target_doc=None):
	...
	if not visit:
		doclist = get_mapped_doc(
			...
			source_name,
			{
				"Rental Sales Order": {"doctype": "Maintenance Visit", "validation": {"docstatus": ["=", 1]}},
				"Rental Sales Order Item": {
...
def get_events(start, end, filters=None):
	...
	conditions = get_event_conditions("Rental Sales Order", filters)

	data = frappe.db.sql(
		"""
		select
			distinct `tabRental Sales Order`.name, `tabRental Sales Order`.customer_name, `tabRental Sales Order`.status,
			`tabRental Sales Order`.delivery_status, `tabRental Sales Order`.billing_status,
			`tabRental Sales Order Item`.delivery_date
		from
			`tabRental Sales Order`, `tabRental Sales Order Item`
		where `tabRental Sales Order`.name = `tabRental Sales Order Item`.parent
			and `tabRental Sales Order`.skip_delivery_note = 0
			and (ifnull(`tabRental Sales Order Item`.delivery_date, '0000-00-00')!= '0000-00-00') \
			and (`tabRental Sales Order Item`.delivery_date between %(start)s and %(end)s)
			and `tabRental Sales Order`.docstatus < 2
			{conditions}
		""".format(
...
def set_delivery_date(items, rental_sales_order):
	delivery_dates = frappe.get_all(
		"Rental Sales Order Item", filters={"parent": rental_sales_order}, fields=["delivery_date", "item_code"]
	)
...
def make_inter_company_purchase_order(source_name, target_doc=None):
	...
	return make_inter_company_transaction("Rental Sales Order", source_name, target_doc)
...
def update_produced_qty_in_so_item(rental_sales_order, rental_rental_sales_order_item):
	...
	frappe.db.set_value("Rental Sales Order Item", rental_rental_sales_order_item, "produced_qty", total_produced_qty)


@frappe.whitelist()
def get_work_order_items(rental_sales_order, for_raw_material_request=0):
	"""Returns items with BOM that already do not have a linked work order"""
	if rental_sales_order:
		so = frappe.get_doc("Rental Sales Order", rental_sales_order)
...
def make_approved(docname):
    ...
    # Use the additional_arg as needed in your logic
    # For example, you can set a field value based on the additional_arg
    # doc.some_field = additional_arg
