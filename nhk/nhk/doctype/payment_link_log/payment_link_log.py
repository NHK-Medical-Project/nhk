    ...
    if p_id:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                sales_order_doc.custom_razorpay_payment_link_log_id = None
...
def sync_payment(link_id, p_id):
    ...
    try:
        ...
        if payment_status == 'paid':
            ...
            sales_order_idsssss = frappe.db.get_value('Payment Link Log', p_id, 'sales_order')
            if sales_order_idsssss:
                frappe.db.set_value('Sales Order', sales_order_idsssss, {
...
def get_razorpay_payment_details(received_amount, sales_order_id, customer_id, link_id,razorpay_payment_ids,p_id):
    try:
        ...
        if order_type == "Rental":
            ...
            payment_entry = create_payment_entry(rounded_total, sales_order_id, customer_id, razorpay_payment_ids, received_amount, master_order_id)

            if security_deposit > 0:
                # frappe.set_user("Administrator")
                journal_entry = create_journal_entry_razorpay(security_deposit, sales_order_id, customer_id, razorpay_payment_ids, master_order_id)
                # frappe.set_user("Guest")

            create_razorpay_payment_details(payment_entry, journal_entry, sales_order_id, order_type, customer_id, razorpay_payment_ids, razorpay_link_so,p_id)
...
def create_razorpay_payment_details(payment_entry_id, journal_entry_id, sales_order_id, order_type, customer, razorpay_payment_ids, razorpay_link_so,p_id):
    try:
        if is_razorpay_payment_details(razorpay_payment_ids):
        ...
        razorpay_payment_details = frappe.get_doc({
            ...
            "reference_id": razorpay_payment_ids
        })
...
def create_journal_entry_razorpay(security_deposit, sales_order_id, customer,razorpay_payment_ids,master_order_id):
    try:
        # frappe.set_user("Administrator")
...
def create_payment_entry(rounded_total, sales_order_id, customer_id, razorpay_payment_ids, received_amount, master_order_id):
    try:
        # frappe.msgprint("Payment Entry Function Called")
        # frappe.set_user("Administrator")
        
        if is_payment_entry_exists(razorpay_payment_ids):
        ...
        payment_link_log = frappe.get_all("Payment Link Log", filters={"link_id": razorpay_payment_ids})
        if payment_link_log:
